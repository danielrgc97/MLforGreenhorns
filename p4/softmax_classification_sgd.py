...
def main(args):
    ...
    X_train, X_test, t_train, t_target = sklearn.model_selection.train_test_split(
        data, target, stratify=target, test_size=args.test_size, random_state=args.seed)

    # Generate initial linear regression weights
    W = generator.uniform(size=[X_train.shape[1], args.classes], low=-0.1, high=0.1)
    ...
    for iteration in range(args.iterations):
        ...
        grads = np.ones([X_train.shape[1], args.classes, args.batch_size])
        ...
        for i in range(X_train.shape[0]):
            ...
            for k in range(args.classes):
                if max < W[:,k] @ X_train[j]: max = W[:,k] @ X_train[j]
            sum = 0
            for k in range(args.classes):
                sum += np.exp(W[:,k]@X_train[j] - max)

            for k in range(args.classes):
                grads[:,k,iCorr] = (t_train[j] - np.exp(W[:,k]@X_train[j] - max)/sum) * X_train[j]

            if iCorr == args.batch_size - 1:
                G = - np.sum(grads,axis=2)/args.batch_size
                W = W - args.learning_rate * G
